...
handle_cast({flush, Count }, MyState) ->
    sign_api:delete_firstN_msgs(MyState#monitor.signs, Count, fun process_to_archive/4),     
    {noreply, MyState};
    
handle_cast( {archive_mysql_start, User, Pwd, Base, MaxSize, Interval }, MyState) ->
    ?LOG_DEBUG("start archiving ~p ~n", [MyState]),  
    timer:apply_interval(Interval, api_table_holder, archive, [ MyState#monitor.signs, MaxSize] ),
...
%% TODO change NOW() to the value of ets table                     
    emysql:prepare(stmt_arhive, 
                 <<"INSERT INTO signs(sign, pub_date) VALUES(?,?)">>),
    {noreply, MyState}.
